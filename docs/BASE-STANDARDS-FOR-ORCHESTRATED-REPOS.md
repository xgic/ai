...
- **Assignee (mandatory at create and before close):** Every issue and PR must be assigned in the same create command (public default **`@xgic`**). Empty assignee after create is a process defect: assign immediately. Before close, `gh issue view --json assignees` (or the PR equivalent); if empty and no written exception, assign then close. Never close then assign. Exception table: [work-tracking-and-checklists.md](work-tracking-and-checklists.md). GitHub cannot require an assignee as a native close rule; this is layered process. Optional `issues: closed` / `pull_request: closed` repair workflows are follow-up only, not a substitute.
- **Checklist completion before close (mandatory):** Before closing any issue or PR that contains Markdown task lists (`- [ ]` / `- [x]`), verify every required item, update the body to mark completed items `- [x]`, and do not close with unchecked required items unless a human documents an explicit waiver. Also confirm assignee is set (or a written exception applies). Reviewers (human or future AI) apply the same gate. See [AGENTS.md](../AGENTS.md).
