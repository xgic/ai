...
5. **Dev Containers** may wrap the same Compose services for IDE parity.  
6. **Named volumes** for durable data; document backup expectations in the product repo.

## Agent checklist

- [ ] Compose file(s) validated (`docker compose config`)  
...
Product repositories own the concrete Compose files; this hub owns the **pattern**.
