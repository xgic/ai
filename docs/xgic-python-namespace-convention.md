...
| GitLab CLI | CLI module | `xgic.cli.gitlab` | [xgic/gitlab-cli](https://github.com/xgic/gitlab-cli) · PyPI [`xgic-gitlab-cli`](https://pypi.org/project/xgic-gitlab-cli/) **0.1.1** | 2 | Core CLI, GraphQL client | **Experimental** — `xgic gitlab` `info` / `health` / `backup` / `restore`; missing ACTION prints full usage |
